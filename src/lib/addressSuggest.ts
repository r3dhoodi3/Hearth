...
// northwest corner down to San Clemente in the southeast. Wider than the nine
// launch cities on purpose - the bbox is a cheap server-side prefilter that
// keeps Photon from ranking a Bolivian village above a Bolsa Avenue address,
// and the exact launch-city gate is applied below on the way out.
...
// number, bus stops, and anything outside the nine launch cities. Asking for
// five raw results routinely left one usable suggestion, or none.
...
//   2. No ZIP, or a ZIP outside the nine launch cities. The ZIP is what every
...
//      be rejected on the very next screen has no business being offered.
...
export function mapPhotonResults(
  ...
  for (const feature of features) {
    ...
    // Prefer the place name OSM has for the address when it is itself a launch
    // city: along the Westminster/Garden Grove line the two disagree, and the
    // name on the mailbox is the one the resident recognizes. Otherwise the
    // ZIP decides, so the city shown can never contradict the ZIP the rest of
    // the app gates on.
