...
//   - no photo IMAGES, only a count: the images are on OakTend, and a dozen
...
function PrintSystemDetail({
  ...
  if (photoCount > 0) {
    facts.push({
      term: "Photos on file",
      value: `${photoCount} photo${photoCount === 1 ? "" : "s"} (in OakTend)`,
    });
  }
    ...
    <div className="mb-5 break-inside-avoid border-b border-stone-200 pb-4 last:border-b-0">
      ...
      {cost && (
        ...
        <p className="mt-1 text-xs text-stone-500">
          OakTend estimate for replacement: ${cost.low.toLocaleString()} to $
      ...
      {!plus && (
        ...
        >
          {Array.from({ length: 8 }).map((_, row) => (
            ...
            >
              OakTend Plus preview &nbsp; OakTend Plus preview &nbsp; OakTend Plus
              preview
            </p>
          ))}
        </div>
