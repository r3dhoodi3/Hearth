...
4. `88772a2` Round 4 (from the 10-checker pass): claim path no longer trusts hidden parcel fields; Ask Hearth clear-while-streaming guard, cross-tab clear, empty-answer refund + idempotent refunds, abort-refund cap (5/hour), pro Ask photo gate (Pro only), trial copy; payments: trial reserved before Stripe checkout (no double trials), webhook cannot resurrect a canceled sub, free/Plus/trial numbers interpolated from one constant, dunning id guard; desktop header overlap fixed at 1024+ (address truncates instead of colliding); phone job cards 44px controls, post-job banner scrolls clear of the header, tip box shown once; app guide no longer re-opens after a tab change; pro copy/routing/legal/a11y fixes; 0140 direct-request block gate.
