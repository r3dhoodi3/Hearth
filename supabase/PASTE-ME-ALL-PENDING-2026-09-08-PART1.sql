-- =============================================================================
-- PASTE-ME-ALL-PENDING-2026-09-08-PART1.sql   (built by Fable 2026-09-08)
-- PART 1: 0154 then 0155-0161. The address lock (0162) is NOT in here; it raised on
-- 7 duplicate-address groups across different owners. Run this file now, then
-- run DUPLICATE-HOMES-QUERY-2026-09-08.sql and send Fable the output.
-- Live is through 0153. Paste the WHOLE file into the Supabase SQL editor, run once.
-- It is one transaction: if any section raises, nothing is applied; read the
-- message, fix the cause, re-run the whole file. Every section is idempotent.
-- BEFORE RUNNING: delete the two throwaway accounts, or the LAST section
-- (0162, one owner per address) can raise:
--   delete from auth.users where email in ('hearth-test-ho-20260907@example.com','hearth-test-pro-20260907@example.com');
-- Part A = PASTE-ME-ALL-PENDING-2026-09-03.sql (0154), Part B = PASTE-ME-ALL-PENDING-2026-09-07.sql (0155-0162), unchanged.
-- =============================================================================

-- ############################## PART A: 0154 ##################################

-- PASTE-ME-ALL-PENDING-2026-09-03.sql
-- Everything not yet on live as of 2026-09-03: migration 0154 only (live is through 0153).
-- Paste the whole file into the Supabase SQL editor and run once. Safe to re-run.

-- PRECHECK: 0110 must already be applied (the function this file replaces must exist).
do $$ begin
  if not exists (select 1 from pg_proc p join pg_namespace n on n.oid = p.pronamespace where n.nspname = 'public' and p.proname = 'enforce_properties_home_cap') then
    raise exception 'PRECHECK FAILED: public.enforce_properties_home_cap missing - apply 0107-0110 first';
  end if;
end $$;
