...
--
-- TWO APP PATHS CHANGE BEHAVIOUR THE MOMENT THIS LANDS, both of them writes
-- that should never have been on a session client:
--   * /account -> toggling the SMS-consent checkbox is refused until
--     saveAccountAction moves that write to the admin client. Saving name and
--     phone with the checkbox untouched keeps working (the guard compares
--     with IS DISTINCT FROM, so an unchanged value is not a change).
--   * the "Invite a neighbor" link stops generating until
--     getOrCreateReferralCode moves its write to the admin client. It already
--     swallows every error and returns null, so nothing crashes - the share
